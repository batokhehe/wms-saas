...
class Category {
  ...
  const Category({required this.id, required this.code, required this.name, required this.status, required this.createdAt});
  factory Category.fromJson(Map<String, dynamic> json) => Category(id: json['id'], code: json['code'], name: json['name'], status: json['status'], createdAt: json['created_at']);
}
...
class CategoryRepository {
  ...
  Future<List<Category>> list() async {
    ...
    return (response.data['data'] as List).map((item) => Category.fromJson(item)).toList();
  }
}

final categoryRepositoryProvider = Provider((ref) => CategoryRepository(ref.read(network.apiClientProvider)));

class CategoryListPage extends ConsumerWidget {
  const CategoryListPage({super.key});
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(FutureProvider((ref) => ref.read(categoryRepositoryProvider).list()));
    return state.when(
