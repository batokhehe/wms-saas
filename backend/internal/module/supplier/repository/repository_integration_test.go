...
// Repository tests run against a real PostgreSQL instance. They prove the
// aggregate survives a round trip through a separate persistence model, and the
// things only a real database can: the company-scoped unique code index, the
...
func (f *fixture) build(t *testing.T, companyID uuid.UUID, code, name string) *entity.Supplier {
	t.Helper()
	supplierCode, err := entity.NewSupplierCode(code)
	if err != nil {
		t.Fatalf("NewSupplierCode(%s) = %v", code, err)
	}
	s, err := entity.NewSupplier(uuid.New(), companyID, supplierCode, name,
...
func (f *fixture) save(t *testing.T, s *entity.Supplier) *entity.Supplier {
	t.Helper()
	if err := f.repo.Save(context.Background(), s); err != nil {
		t.Fatalf("Save() = %v", err)
	}
	return s
}
...
func appliedPaging(t *testing.T) pagination.Request {
	...
	if err := req.Apply(pagination.Options{
		...
		AllowedSorts: map[string]string{"code": "suppliers.code"},
	}); err != nil {
...
func TestAggregateSurvivesRoundTrip(t *testing.T) {
	...
	s := f.build(t, f.acme, "SUP-1", "Acme Traders")
	...
	if err := s.Update("Acme Traders International", email, phone, tax, addr, f.actor, now); err != nil {
		t.Fatal(err)
	}
	s.Deactivate(f.actor, now)
	f.save(t, s)

	loaded, err := f.repo.FindByID(ctx, s.ID(), f.acme)
	if err != nil {
		t.Fatalf("FindByID() = %v", err)
	}
	if loaded.Code().String() != "SUP-1" || loaded.Name() != "Acme Traders International" {
...
func TestUnsetContactStoredAsNull(t *testing.T) {
	f := newFixture(t)
	s := f.save(t, f.build(t, f.acme, "SUP-1", "No Contact"))

	var email, phone *string
	f.db.Raw("SELECT email FROM suppliers WHERE id = ?", s.ID()).Scan(&email)
	f.db.Raw("SELECT phone FROM suppliers WHERE id = ?", s.ID()).Scan(&phone)
	if email != nil || phone != nil {
		t.Errorf("unset contact stored as non-NULL: email=%v phone=%v", email, phone)
	}

	loaded, _ := f.repo.FindByID(context.Background(), s.ID(), f.acme)
...
func TestConcurrentUpdateAllowsExactlyOneWriter(t *testing.T) {
	f := newFixture(t)
	ctx := context.Background()
	s := f.save(t, f.build(t, f.acme, "SUP-1", "Acme"))

	left, err := f.repo.FindByID(ctx, s.ID(), f.acme)
	if err != nil {
		t.Fatal(err)
	}
	right, err := f.repo.FindByID(ctx, s.ID(), f.acme)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, candidate := range []*entity.Supplier{left, right} {
		wg.Add(1)
		go func(c *entity.Supplier) { defer wg.Done(); <-start; errs <- f.repo.Update(ctx, c) }(candidate)
	}
	close(start)
	wg.Wait()
	close(errs)
	...
	loaded, _ := f.repo.FindByID(ctx, s.ID(), f.acme)
	if loaded.Version() != 2 {
...
func TestReadIsIsolated(t *testing.T) {
	f := newFixture(t)
	globexSupplier := f.save(t, f.build(t, f.globex, "SUP-1", "Globex"))

	_, err := f.repo.FindByID(context.Background(), globexSupplier.ID(), f.acme)
...
func TestListIsIsolated(t *testing.T) {
	f := newFixture(t)
	f.save(t, f.build(t, f.acme, "SUP-1", "Acme One"))
	f.save(t, f.build(t, f.acme, "SUP-2", "Acme Two"))
	f.save(t, f.build(t, f.globex, "SUP-1", "Globex One"))
	...
	for _, s := range page.Items {
		if s.CompanyID() != f.acme {
			t.Errorf("list leaked company %s", s.CompanyID())
		}
	}
}
...
func TestSameCodeAllowedAcrossCompanies(t *testing.T) {
	f := newFixture(t)
	f.save(t, f.build(t, f.acme, "SUP-1", "Acme"))
	if err := f.repo.Save(context.Background(), f.build(t, f.globex, "SUP-1", "Globex")); err != nil {
...
func TestExistsByCodeIsIsolated(t *testing.T) {
	f := newFixture(t)
	ctx := context.Background()
	f.save(t, f.build(t, f.globex, "SUP-1", "Globex"))

	taken, err := f.repo.ExistsByCode(ctx, f.acme, "SUP-1")
	if err != nil {
		t.Fatal(err)
	}
	if taken {
...
func TestDuplicateCodeIsConflict(t *testing.T) {
	...
	err := f.repo.Save(context.Background(), f.build(t, f.acme, "sup-1", "Acme Two"))
...
func TestStatusCheckConstraint(t *testing.T) {
	f := newFixture(t)
	// Written directly, bypassing the aggregate, to prove the DATABASE refuses an
	// invalid status.
	err := f.db.Exec(`
		INSERT INTO suppliers (id, company_id, code, name, status, created_by, updated_by, created_at, updated_at)
		VALUES (?, ?, 'SUP-9', 'Bad Status', 'ARCHIVED', ?, ?, NOW(), NOW())`,
...
func TestForeignKeyRejectsUnknownCompany(t *testing.T) {
	f := newFixture(t)
	err := f.repo.Save(context.Background(), f.build(t, uuid.New(), "SUP-9", "Ghost"))
	if err == nil {
		t.Fatal("a supplier for a nonexistent company was accepted")
	}
}
...
func TestListFiltersByStatusAndFindByCode(t *testing.T) {
	...
	f.save(t, f.build(t, f.acme, "SUP-1", "Active One"))
	inactive := f.build(t, f.acme, "SUP-2", "Inactive One")
	...
	if page.Meta.Total != 1 || page.Items[0].Code().String() != "SUP-2" {
		t.Fatalf("status filter = %+v, want only SUP-2", page.Items)
	}

	found, err := f.repo.FindByCode(ctx, f.acme, "sup-1")
	if err != nil {
...
func TestSaveRollsBack(t *testing.T) {
	...
	err := manager.RunInTransaction(ctx, func(txCtx context.Context) error {
		if err := f.repo.Save(txCtx, f.build(t, f.acme, "SUP-9", "Rolled Back")); err != nil {
			return err
		}
		return sentinel
	})
	...
	taken, err := f.repo.ExistsByCode(ctx, f.acme, "SUP-9")
	if err != nil {
		t.Fatal(err)
	}
	if taken {
		t.Error("a supplier survived a rolled-back transaction")
	}
}

// ---------- RBAC seed ----------

func TestSupplierPermissionsAreSeeded(t *testing.T) {
	f := newFixture(t)
	var codes []string
	f.db.Raw("SELECT code FROM permissions WHERE module = 'supplier' ORDER BY code").Scan(&codes)
	want := []string{"supplier.activate", "supplier.create", "supplier.read", "supplier.update"}
	if len(codes) != len(want) {
		t.Fatalf("seeded %d supplier permissions, want %d: %v", len(codes), len(want), codes)
	}
	for i := range want {
		if codes[i] != want[i] {
