...
// EventPublisher publishes purchase-order domain events. Declared here in the
// consumer, matching every other module.
type EventPublisher interface {
	Publish(ctx context.Context, event entity.Event)
}

// LogEventPublisher writes purchase-order events to the structured log.
//
...
// commits, so an event is only ever published for a transition that persisted.
...
func (p *LogEventPublisher) Publish(ctx context.Context, event entity.Event) {
	...
	fields := []zap.Field{
		...
		zap.String("event_order_id", event.OrderID.String()),
