...
// EventPublisher publishes goods-receipt domain events. Declared here in the
// consumer, matching every other module.
type EventPublisher interface {
	Publish(ctx context.Context, event entity.Event)
}

// LogEventPublisher writes goods-receipt events to the structured log.
//
// Publishing into a broker with no subscriber would fill an operator's dashboard
// with dead-task errors, so until a real consumer exists the durable audit trail
// is the log. These events are raised by the AGGREGATE and pulled by the service
// AFTER the transaction commits, so an event is only ever published for a
// transition that persisted.
...
func (p *LogEventPublisher) Publish(ctx context.Context, event entity.Event) {
	// Every event field is prefixed "event_", including the identifiers, so they
	// do not collide with the ones the request-scoped logger already carries.
	fields := []zap.Field{
		zap.String("event", string(event.Name)),
		zap.String("event_receipt_id", event.ReceiptID.String()),
