...
// EventPublisher publishes warehouse domain events.
//
// Declared here in the consumer, matching the other modules.
type EventPublisher interface {
	Publish(ctx context.Context, event entity.Event)
}

// LogEventPublisher writes warehouse events to the structured log.
//
...
//
// These events differ from the earlier modules' in one respect worth noting:
// they are raised by the AGGREGATE, not by the service. The service publishes
// what it pulls, so an event exists because a transition happened rather than
// because a service method remembered to record one.
